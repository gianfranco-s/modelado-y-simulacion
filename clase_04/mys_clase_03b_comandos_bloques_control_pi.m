...
vel_antena = feedback(tf_dir, retroalim_interna);  % rapidez angular

% Expresión usando forma reducida analítica
s = tf("s");
Jeq = Jm+N^2*JL;
beq = bm+N^2*bL;
vel_antena_verdadera = N*kt/ ((Jeq*s + beq)*(La*s + Ra) + kt*kb);

pos_antena = (1/s) * vel_antena_verdadera;
